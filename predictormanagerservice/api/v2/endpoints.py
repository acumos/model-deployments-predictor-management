...
class PredictorCollection(Resource):
    ...
    @api.response(500, 'Unexpected Error: The Predictor was not created')
    @api.expect(create_fields)
    def post(self):
        """
        Creates a new Predictor with the supplied JSON.

        Use this method to create a Predictor.
...
class JobHistory(Resource):
    ...
    @api.response(500, 'Unexpected internal server error')
    def get(self, predictorKey, scheduleId):
        """
